...
@if (Session::has('failed_login_locktime'))
    @php
        $lockoutTime = Session::get('failed_login_locktime');
        $remainingTime = $lockoutTime->diff(now());
    @endphp

    <div class="p-2 w-100 m-auto" style="max-width: 330px; padding-top: 10% !important;">
        <div class="alert alert-danger" role="alert">
            You are locked out. Please try again in {{ $remainingTime->format('%h hours %i minutes %s seconds') }}.
        </div>
    </div>
@else
    @if ($errors->any())
